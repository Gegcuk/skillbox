def ex20_2_1():
    ...
    requesting_product = input('What product are looking for? ')
    if big_storage.get(requesting_product) is None:
        print('Product is not existing')
    else:
        print(big_storage.get(requesting_product))
...
def ex20_2_2():
    ...
    print('Total income: {}'.format(sum(incomes.values())))
    min_income = min(incomes.values())
    min_key = ''
    for key in incomes.keys():
        if incomes.get(key) == min_income:
            print('{} has least income: {}'.format(key, min_income))
            min_key = key
    incomes.pop(min_key)
    print(incomes)
...
def ex20_2_3():
    ...
    for sym in text:
        if sym in count_dict.keys():
            ...
            count_dict[sym] = 1
    print('Max frequency: {}'.format(max(count_dict.values())))
    for key in sorted(count_dict):
        print('{} : {}'.format(key, count_dict.get(key)))
